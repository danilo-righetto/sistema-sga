...
class Veiculo extends CI_Controller {
    ...
    public function index($indice = null) {
        ...
        if ($indice == 1) {
            ...
            $data['msg'] = "Não foi possível cadastrar o veículo!";
            $this->load->viem('includes/msg_erro', $data);
        } else if ($indice == 3) {
            $data['msg'] = "Veículo excluído com sucesso!";
            $this->load->view('includes/msg_sucesso', $data);
        } else if ($indice == 4) {
            $data['msg'] = "Não foi possível excluir o veículo!";
            $this->load->viem('includes/msg_erro', $data);
        } else if ($indice == 5) {
            $data['msg'] = "Veículo atualizado com sucesso!";
            $this->load->view('includes/msg_sucesso', $data);
        } else if ($indice == 6) {
            $data['msg'] = "Não foi possível atualizar o veículo!";
            $this->load->viem('includes/msg_erro', $data);
        }
